...
const drawerLinkClasses = 'flex cursor-pointer items-center rounded-xl px-3 py-2.5 hover:bg-[#f7f9ff]'

function HeaderLink({ item, onClick, active, className }: { item: HeaderNavItem; onClick?: () => void; active?: boolean; className?: string }) {
...
export function AppHeader({ onMenuClick, mobileMenuOpen = false }: AppHeaderProps) {
  ...
  const closeMenu = () => {
    setMobileOpen(false)
    if (usesExternalMenu) onMenuClick?.()
  }
          ...
          <div className="flex h-full flex-col overflow-hidden bg-white shadow-[0_12px_36px_rgba(7,31,82,0.06)]">
            <div className="flex items-center justify-between border-b border-[#071f52]/10 px-4 py-3">
              <div className="flex min-w-0 items-center gap-2.5">
                <img src={settings?.logo_url || '/logo.jpg'} alt={settings?.business_name || 'Katada Transportation Services'} className="h-8 w-8 shrink-0 rounded-xl object-cover ring-1 ring-[#071f52]/10" />
                <p className="min-w-0 truncate text-xs font-black text-[#071f52]">{settings?.business_name || 'Katada Van Rentals'}</p>
              </div>
              <button
                type="button"
            ...
            <nav className="flex-1 overflow-y-auto p-3">
              {topLinks.map((item) => (
                <HeaderLink key={item.label} item={item} onClick={closeMenu} className={drawerLinkClasses} active />
              ))}
  
              {!user ? (
                <div className="mt-3 space-y-2 border-t border-[#071f52]/10 pt-3">
                  <Link to="/login" onClick={closeMenu} className="block rounded-2xl border border-[#071f52]/10 px-4 py-3 text-sm font-bold text-[#071f52]">
                    Sign in
                  </Link>
                  <Link to="/register" onClick={closeMenu} className="block rounded-2xl bg-[#e92935] px-4 py-3 text-sm font-bold text-white">
                    Book Now
                  </Link>
                </div>
              ) : (
                <div className="mt-3 space-y-1 border-t border-[#071f52]/10 pt-3">
                  {accountLinks.map((item) => (
                    <HeaderLink key={item.to} item={item} onClick={closeMenu} className={drawerLinkClasses} />
                  ))}
                  <button
                    type="button"
                    onClick={handleSignOut}
                    className="mt-1 flex w-full items-center gap-2 rounded-2xl px-3 py-2.5 text-sm font-bold text-[#e92935] transition-colors hover:bg-[#fff5f5]"
                  >
                    <LogOut size={16} /> Logout
                  </button>
                </div>

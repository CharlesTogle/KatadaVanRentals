      ...
      <div className="mt-10 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <article className="group overflow-hidden rounded-[30px] bg-[#071f52] text-white shadow-[0_24px_70px_rgba(7,31,82,0.24)]">
          <div className="grid min-h-full gap-0 md:grid-cols-2">
            <img src={fleet[0].image} alt={`${fleet[0].name} interior`} className="h-full min-h-[260px] w-full object-cover transition-transform duration-700 group-hover:scale-[1.03] sm:min-h-[320px]" />
            <div className="flex flex-col justify-between p-6 sm:p-8">
              <div>
                <div className="mb-5 inline-flex items-center gap-2 rounded-full bg-[#ffd923] px-4 py-2 text-sm font-black text-[#071f52]">
                  <Sparkles size={16} aria-hidden="true" />
                  Most comfortable
                </div>
                <h3 className="text-3xl font-black tracking-[-0.04em]">{fleet[0].name}</h3>
                <p className="mt-3 text-sm leading-6 text-white/72 sm:text-base sm:leading-7">{fleet[0].note}</p>
              </div>
        ...
        <div className="grid gap-5">
          {fleet.slice(1).map((van) => (
            <article key={van.name} className="group grid overflow-hidden rounded-[30px] border border-[#071f52]/10 bg-white shadow-[0_16px_44px_rgba(7,31,82,0.08)] sm:grid-cols-[0.42fr_0.58fr]">
              <img src={van.image} alt={`${van.name} interior`} className="h-44 w-full object-cover transition-transform duration-700 group-hover:scale-[1.03] sm:h-full" />
              <div className="p-5 sm:p-6">
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs font-black text-[#071f52]">

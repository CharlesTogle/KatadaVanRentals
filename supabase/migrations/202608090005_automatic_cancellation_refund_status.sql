...
begin
  ...
  with expired_candidates as materialized (
    select id, status as previous_status, rental_model
  ...
  expired as (
    ...
    where b.id = c.id
    returning b.id, c.previous_status, c.rental_model
  ),
  cancellation_records as (
    insert into public.booking_cancellations (booking_id, cancellation_type, reason, refund_status)
    select id,
           'system_expiry',
           ...
           case when rental_model in ('all_in', 'all_out') then 'pending_refund' else 'refund_cancelled' end
    from expired

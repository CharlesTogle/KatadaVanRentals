...
begin
  ...
  set total_amount = adjusted_total,
      ...
      price_line_items = case when was_manual then jsonb_build_array(
        jsonb_build_object('label', 'Base', 'detail', 'Manual pricing', 'amount', adjusted_total)
      ) else price_line_items end,
      updated_at = now()
  where id = target_booking_id;

...
begin
  ...
  if p_end_at is null then
    -- Dropoff booking: vehicle is unavailable if any non-terminal booking exists
    ...
    if found then
      raise exception 'Vehicle is not available. It has an active booking (#%).', v_conflict.booking_number;
    end if;
  else
    ...
    if found then
      raise exception 'Vehicle is not available for these dates. It has a conflicting booking (#%).', v_conflict.booking_number;
    end if;
  end if;
